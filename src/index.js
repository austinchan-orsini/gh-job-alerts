 ...
 *      (publishes new-job messages to SQS — see poller.js)
 *   3. node-cron scheduler that calls runDailyDigest() on DIGEST_CRON schedule
 *
 * The Discord bot and the SMS/Discord/bot alert fan-out both live in the
 * separate alert-worker.js process ("node src/alert-worker.js"), which
 * consumes the messages this process's poller publishes.
 */
